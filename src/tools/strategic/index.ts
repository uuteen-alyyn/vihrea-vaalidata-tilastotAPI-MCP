...
export function registerStrategicTools(server: McpServer): void {
  ...
  server.tool(
    ...
    'Identifies candidates who ran in from_year but did not run in to_year (inactive candidates). Returns their votes, strongest areas, and party — useful for identifying "orphaned" vote pools that may be available for other candidates. Both years must have candidate data in the registry.',
    {
      ...
      vaalipiiri: z.string().describe('Vaalipiiri key (e.g. "helsinki"). Candidate data is stored per vaalipiiri.'),
    ...
    async ({ from_year, to_year, vaalipiiri, party_id, min_votes = 100, limit = 20 }) => {
      // Load candidate metadata (names only — no need to fetch full data rows) from both years
      // Use metadata fetch to check available candidate IDs cheaply
      const fromTables = getElectionTables('parliamentary', from_year);
      const toTables = getElectionTables('parliamentary', to_year);

      if (!fromTables?.candidate_by_aanestysalue?.[vaalipiiri]) {
        return errResult(`No candidate data for parliamentary ${from_year} vaalipiiri "${vaalipiiri}". Registered years with candidate data: ${
          ['parliamentary'].flatMap(() => [2019, 2023]).join(', ')
        }.`);
      }
      if (!toTables?.candidate_by_aanestysalue?.[vaalipiiri]) {
        return errResult(`No candidate data for parliamentary ${to_year} vaalipiiri "${vaalipiiri}". Registered years with candidate data: ${
          ['parliamentary'].flatMap(() => [2019, 2023]).join(', ')
        }.`);
      }
      ...
      try {
        const fromResult = await loadCandidateResults(from_year, vaalipiiri);
      ...
      try {
        const toResult = await loadCandidateResults(to_year, vaalipiiri);
      ...
      // Get vaalipiiri-level totals from from_year for each candidate
      const fromVpRows = fromRows.filter((r) => r.area_level === 'vaalipiiri' && r.candidate_id);
      ...
      if (inactive.length === 0) {
        return mcpText({
          from_year, to_year, vaalipiiri, party_id: party_id ?? 'all',
          inactive_candidates: [],
      ...
      return mcpText({
        ...
        method: {
          description: 'Inactive = candidate name present in from_year but absent in to_year (normalized name match — candidate IDs differ between elections). Votes from vaalipiiri-level aggregate row.',
  ...
  server.tool(
    'find_exposed_vote_pools',
    'Identifies areas with politically "available" votes based on party vote changes between two elections. An exposed pool is an area where a party lost a significant share of its vote between elections, suggesting those voters may be looking for alternatives. Works with party data (13sw, all years 1983–2023).',
    {
      party_id: z.string().describe('The party whose lost votes represent an opportunity (e.g. opponent party, or a party that collapsed).'),
      year1: z.number().describe('The earlier election year (baseline).'),
      year2: z.number().describe('The later election year (comparison).'),
      min_vote_loss_pp: z.number().optional().describe('Minimum vote share loss in percentage points to flag an area as exposed. Defaults to 3.'),
      min_votes_year1: z.number().optional().describe('Minimum votes in year1 to include an area (filters tiny areas). Defaults to 50.'),
      limit: z.number().optional().describe('Maximum areas to return, sorted by vote loss descending. Defaults to 30.'),
    },
    async ({ party_id, year1, year2, min_vote_loss_pp = 3, min_votes_year1 = 50, limit = 30 }) => {
      let rows1: ElectionRecord[];
      let rows2: ElectionRecord[];
      let tableId: string;

      try {
        const r1 = await loadPartyResults(year1);
        rows1 = r1.rows;
        tableId = r1.tableId;
      } catch (err) {
      ...
      try {
        const r2 = await loadPartyResults(year2);
        rows2 = r2.rows;
      } catch (err) {
        return errResult(`Failed to load ${year2} data: ${String(err)}`);
      }

      const kunta1 = rows1.filter((r) => matchesParty(r, party_id) && r.area_level === 'kunta' && r.votes >= min_votes_year1);
      if (kunta1.length === 0) return errResult(`Party "${party_id}" not found in ${year1}.`);

      // Build lookup for year2
      const kunta2Map = new Map<string, ElectionRecord>();
      rows2.filter((r) => matchesParty(r, party_id) && r.area_level === 'kunta')
      ...
      return mcpText({
        ...
        strategic_note: `These ${exposed.length} municipalities saw ${party_id} lose ≥${min_vote_loss_pp}pp between ${year1} and ${year2}. They represent areas where ${party_id} support is structurally weaker and voters may be persuadable.`,
        method: {
          description: 'Exposed = kunta where party vote share fell by ≥ min_vote_loss_pp between year1 and year2. Data from 13sw party-by-kunta table.',
  ...
  server.tool(
    ...
    'Estimates where votes moved between two elections using area-level vote changes as a structural proxy. Compares how much one party gained vs. how much another lost in the same areas. This is NOT a causal measurement — it is an inference from aggregate election results.',
    {
      ...
      area_id: z.string().optional().describe('Restrict to a specific area (6-digit 13sw code). Omit to analyse at national/kunta level.'),
      min_votes: z.number().optional().describe('Minimum votes in year1 to include a kunta. Defaults to 50.'),
    },
    async ({ losing_party_id, gaining_party_id, year1, year2, area_id, min_votes = 50 }) => {
      let rows1: ElectionRecord[];
      let rows2: ElectionRecord[];
      let tableId: string;

      try {
        const r1 = await loadPartyResults(year1, area_id);
        rows1 = r1.rows;
        tableId = r1.tableId;
      } catch (err) {
        return errResult(`Failed to load ${year1} data: ${String(err)}`);
      ...
      try {
        const r2 = await loadPartyResults(year2, area_id);
        rows2 = r2.rows;
      } catch (err) {
        return errResult(`Failed to load ${year2} data: ${String(err)}`);
      }

      const level = area_id ? undefined : 'kunta';
      ...
      return mcpText({
        ...
        method: {
          description: 'Area co-movement analysis: for each kunta, check if losing_party votes fell AND gaining_party votes rose between year1 and year2.',
          source_table: tableId,
          proxy_method: 'election result inference',
          confidence: 'structural indicator',
        },
  ...
  server.tool(
    ...
    async ({ party_id, reference_year, trend_year, min_votes = 20, limit = 25 }) => {
      let refRows: ElectionRecord[];
      let trendRows: ElectionRecord[] | null = null;
      let tableId: string;

      try {
        const r = await loadPartyResults(reference_year);
        refRows = r.rows;
        tableId = r.tableId;
      } catch (err) {
      ...
      if (trend_year) {
        try {
          const r = await loadPartyResults(trend_year);
          trendRows = r.rows;
        } catch (_) {
          trendRows = null; // trend scoring disabled
        }
      }

      // Party national share as reference
      ...
      if (!nationalShare) return errResult(`Party "${party_id}" not found in ${reference_year}.`);

      // Get all-party kunta totals (for electorate size proxy)
      const allKuntaRows = refRows.filter((r) => r.area_level === 'kunta');
      const allVotesByKunta = new Map<string, number>();
      for (const r of allKuntaRows) {
        allVotesByKunta.set(r.area_id, (allVotesByKunta.get(r.area_id) ?? 0) + r.votes);
      }

      // Party-specific kunta rows
      const partyKunta = refRows.filter(
        (r) => matchesParty(r, party_id) && r.area_level === 'kunta' && r.votes >= min_votes
      );
      if (partyKunta.length === 0) return errResult(`No kunta data found for "${party_id}" in ${reference_year}.`);

      // Build trend map
      const trendMap = new Map<string, number>();
      if (trendRows) {
        trendRows
          .filter((r) => matchesParty(r, party_id) && r.area_level === 'kunta')
      ...
      const scored = partyKunta.map((r) => {
        ...
        const totalVotes = allVotesByKunta.get(r.area_id) ?? 0;
      ...
      return mcpText({
        ...
        national_share_pct: pct(nationalShare),
        n_municipalities_scored: scored.length,
        ...
        scoring_methodology: {
          ...
          components: {
            ...
            c3_size: 'Raw party votes relative to the party\'s best kunta. Larger vote pools score higher.',
